...
# JS-based auto-refresh (no dependency)
...
if auto_refresh:
    ...
    st.caption(f"⏱ Auto-refresh ON (JS) — every {interval_s}s · Count: {st.session_state.refresh_count}")
...
with tabs[0]:
    ...
    if not ticker:
        ...
        if df_raw.empty:
            ...
            st.caption("How to read: price above longer SMAs (e.g., SMA200) suggests uptrend. Upper band touches = strength; lower band touches = weakness.")
...
with tabs[1]:
    if not ticker:
        ...
        if dfr.empty:
            ...
            st.caption("How to read: positive bars add to the score (bullish), negative bars subtract (bearish). Heuristic only.")
...
with tabs[2]:
    if not ticker:
        ...
        if dfr.empty:
            ...
            st.caption("Metrics: higher CAGR/Sharpe = better; Max DD more negative = worse. Past performance ≠ future results.")
...
with tabs[9]:
    st.subheader("Policies (customized)")
...
1. **No advisory relationship**: Your Company LLC does not provide personalized investment advice or act as your investment adviser.
2. **No guarantees**: Markets are risky. Your Company LLC does not guarantee performance or outcomes.
...
6. **Limitation of liability**: To the maximum extent permitted by law, Your Company LLC is not liable for any losses or damages arising from your use of this app.
7. **Changes**: Features may change without notice.
8. **Governing law**: The laws of Illinois, USA may apply. Seek legal advice if needed.
