...
import pytz, re, io, math, traceback, unicodedata
...
# Altair globals (update for v5.5 notice)
...
def sanitize_text_ascii(s: str) -> str:
    ...
    for k,v in _ASCII_MAP.items():
        s = s.replace(k,v)
    # strip emojis/non-BMP
    s = _EMOJI_RE.sub("", s)
    # normalize then drop non-latin1
...
# ---------- Tab 1 — Price & Indicators (Yahoo-like with Candles + Volume) ----------
with tabs[0]:
    ...
    if not ticker:
        ...
        if q["last"] is None:
            ...
            if dfi.empty or "Close" not in dfi.columns:
                ...
                # Volume (separate row, Yahoo-like)
                ...
                st.caption("Candles: green up / red down; wicks show H‑L. **Higher price is better** for longs. Volume: **higher bars indicate more activity**.")
...
with tabs[2]:
    ...
    with c1:
        ...
        if isinstance(eps, pd.Series) and len(eps) > 0:
            ...
            st.caption("EPS: **higher is better**; rising trend often constructive. Dashed line = 4‑Q moving average.")
        else:
