...
dark_css = "<style>.appview-container, .main { background-color: #0f172a; color: #e5e7eb; }</style>"
...
# store as list for safety (no set)
if "watchlist_extra" not in st.session_state:
    st.session_state["watchlist_extra"] = []
if isinstance(st.session_state["watchlist_extra"], set):
    st.session_state["watchlist_extra"] = list(st.session_state["watchlist_extra"])
    ...
    if not ticker:
        ...
        if q["last"] is None:
            ...
            if dfi.empty or "Close" not in dfi.columns:
                ...
                if "Date" not in dfx.columns and "Date" in df_raw.columns:
                    dfx["Date"] = pd.to_datetime(df_raw["Date"])
                elif "Date" not in dfx.columns:
                    dfx.insert(0, "Date", pd.to_datetime(dfx.index))
                ...
                if style == "Close line":
                    ...
                    layers.append(base.mark_point(size=20, opacity=0).encode(y="Close:Q").add_params(hover))
                    layers.append(alt.Chart(dfx).mark_rule(color="gray").encode(x="Date:T").transform_filter(hover))
                else:
                    # Candlestick
                    rule = base.mark_rule().encode(
                        y="Low:Q", y2="High:Q", color=alt.condition(alt.datum.Close >= alt.datum.Open, alt.value("#16a34a"), alt.value("#dc2626")),
                        tooltip=["Date:T","Open:Q","High:Q","Low:Q","Close:Q","Volume:Q"]
                    )
                ...
                if show_bb and "BB_LO" in dfx.columns and "BB_UP" in dfx.columns:
                    layers.append(base.mark_area(opacity=0.12).encode(
                ...
                ok = safe_altair_chart(chart_combo, use_container_width=True)
                if not ok:
                    st_line_chart_fallback(dfx, ["Close","SMA_20","SMA_50","SMA_200"], xcol="Date")
                st.caption("Candles: green up / red down; wicks show H‑L. **Higher price is better** for longs. Volume: **higher bars = more activity**.")
                ...
                st.caption("Signals: **more positive** bars suggest bullish alignment; **negative** bars suggest bearish tilt. Heuristic only.")
...
# ---------- Tab 7 — Policies (simplified, PDF-safe) ----------
with tabs[6]:
