...
def sparkline_chart(df: pd.DataFrame, is_dark: bool, field="Close"):
    df2 = df.copy()
    if "Date" not in df2.columns: df2 = df2.reset_index()
    if "Date" not in df2.columns:
        dcols = [c for c in df2.columns if pd.api.types.is_datetime64_any_dtype(df2[c])]
        if dcols: df2.rename(columns={dcols[0]:"Date"}, inplace=True)
        else: df2.insert(0, "Date", pd.to_datetime(df2.index))
    df2["Date"] = pd.to_datetime(df2["Date"])
    if field not in df2.columns:
        for cand in ["Close","Adj Close","AdjClose","Adj_Close","close"]:
            if cand in df2.columns: field = cand; break
    p = palette(is_dark)
    ch = alt.Chart(df2.tail(60)).mark_area(opacity=0.25, line=True, color=p["mid"]).encode(
        ...
        if q["last"] is None:
            ...
            with col1:
                ...
                if isinstance(hist, pd.DataFrame) and not hist.empty:
                    ok = safe_altair_chart(sparkline_chart(hist, is_dark, "Close"), is_dark, use_container_width=True)
            ...
            if dfi.empty or "Close" not in dfi.columns:
                ...
                # Volume
                vol = alt.Chart(dfx).mark_bar(color=p["mid"]).encode(
                    x="Date:T", y=alt.Y("Volume:Q", title="Volume"), tooltip=["Date:T","Volume:Q"]
                ...
                st.caption("Candles: green up / red down; wicks show High–Low. **Higher price is better** for longs. Volume: **higher bars = more activity**.")
            ...
            with cols[i % 3]:
                try:
                    ...
                    if isinstance(df, pd.DataFrame) and not df.empty:
                        ok = safe_altair_chart(sparkline_chart(df, is_dark, "Close"), is_dark, use_container_width=True)
                        if not ok: st_line_chart_fallback(df, ["Close"], xcol="Date")
                except Exception as e:
                    st.caption(f"{sym}: {e}")

# ---------- Tab 5 — Heatmap (Enhanced) ----------
with tabs[4]:
    ...
    if not symbols:
        ...
        for sym in symbols:
            try:
                ...
                # YTD: compare with first trading day of current year
                first_this_year = close[close.index >= pd.Timestamp(f"{pd.Timestamp.today().year}-01-01")]
                if len(first_this_year) >= 1:
                    vals["YTD %"] = (close.iloc[-1] / first_this_year.iloc[0] - 1) * 100
                else:
                    vals["YTD %"] = np.nan
            ...
            if long.empty:
                ...
                if group_by_sector:
                    chart = heat_layer.facet(
                        row=alt.Row("Sector:N", sort='-count', header=alt.Header(labelColor=palette(is_dark)["fg"])),
                    ).resolve_scale(color="shared")
                else:
                    chart = heat_layer.properties(height=220)

                ok = safe_altair_chart(chart, is_dark, use_container_width=True)
                if not ok: st.dataframe(dfh.set_index("Ticker")[metrics], use_container_width=True)
                st.caption("Heatmap: **blue = positive**, **red = negative** relative returns. Optional bubbles show **30‑day avg volume** (larger = higher activity). Group by sector to spot leadership/laggards.")
